...
from alexnet import remove_all_file
class GuidedBackprop(SaliencyMask):
  ...
  def __init__(self, graph, session, y, x,ckpt_path):
    ...
    with graph.as_default():
      # remove_all_file('ckpt_tmp')
      saver = tf.train.Saver()
      # saver.save(session, './ckpt_tmp/guided_backprop.ckpt')
    ...
    with self.guided_graph.as_default():
      ...
      with self.guided_graph.gradient_override_map({'Relu': 'GuidedRelu'}):
        ...
        saver.restore(self.guided_sess, model_file)
