...
if not os.path.isdir('result/'+dataset+'/'+mode+'/Lad_'+ str(L_ad) + '_Lst_'+ str(L_style)+ '_Ltv_'+ str(L_tv)+'_uniform'):
    os.mkdir('result/'+dataset+'/'+mode+'/Lad_'+ str(L_ad) + '_Lst_'+ str(L_style)+ '_Ltv_'+ str(L_tv)+'_uniform')
SAVE_PATH=('result/'+dataset+'/'+mode+'/Lad_'+ str(L_ad) + '_Lst_'+ str(L_style)+ '_Ltv_'+ str(L_tv)+'_uniform')
...
def styleloss_RNFLD(syn,style_gram,weight_gram,sess):
    ...
    :param style_gram: ndarray N,256,256
    :param vessel_fea_dim:  ndarray N,15,
    :param weight_gram:  ndarray N,256,256
    :param sess:
    :param mode:
    :return:
    """
    ...
    syn_gram=tf.matmul(tf.transpose(cnn_output_syn, perm=[0, 2, 1]),cnn_output_syn)#N,256,256
    syn_gram=tf.multiply(weight_gram,syn_gram)
...
def main():
    ...
    with slim.arg_scope(AlexNet.alexnet_v2_arg_scope()):
        k = net.alexnet_v3()
        logits = k[11]
        norm_grads = tf.gradients(logits[:, 1], k[7])[0] # N,55,55,64
    variables = tf.contrib.framework.get_variables_to_restore()[71:85]
    saver_syn = tf.train.Saver(variables)
    model_file1 = tf.train.latest_checkpoint('./ckpt3/')
    saver_syn.restore(sess, model_file1)
    ...
    g_loss = L_ad * g_loss_ad + L_style * g_loss_style + L_tv * g_loss_tv
    ...
    for epoch in range(2400):
        for idx in range(num_batches):
            ...
            weight_gram_temp=np.expand_dims(weights,axis=2)#N,256,1
            weight_gram_temp1=np.matmul(weight_gram_temp,np.transpose(weight_gram_temp, axes=(0,2,1)))#N,256,256
            cnn_out=np.reshape(cnn_out,[-1,cnn_out.shape[1]*cnn_out.shape[2],cnn_out.shape[3]]) #N,6*6,256
            Style_gram=np.matmul(np.transpose(cnn_out, axes=(0,2,1)), cnn_out)#N,256,256
            ...
            if count % int(350/BATCH_SIZE) == 0:
                print(" RimLoss_style_uniform L1=%.2f L2=%.2f L3=%.2f Epoch: [%2d] [%4d/%4d] d_loss: %.6f, g_loss: %.6f" \
                      % (L_ad,L_style,L_tv,epoch, idx, num_batches, errD_fake + errD_real, errG))
            ...
            if count % int(35000/BATCH_SIZE) == 0:
                saver2.save(sess, 'ckpt_gan/RimLoss_style_uniform_Lad_' + str(L_ad) + '_Lst_'+ str(L_style)+ '_Ltv_'+ str(L_tv) +'.ckpt', global_step=count + 1)
